...
//    cout<<quaternion.matrix().eulerAngles(0,1,2)*(180/M_PI)<<endl;
...
//    targetError.linear.x = cOrientation_world.x();
//    targetError.linear.y = cOrientation_world.y();
//    targetError.linear.z = cOrientation_world.z();
...
//        Eigen::Vector3d desiredVectorD = getOrientationD(cDesiredVector_world, pDesiredVector_world);
//        Eigen::Vector3d desiredVectorDD = getOrientationDD(cDesiredVector_world, pDesiredVector_world,
//                                                           ppDesiredVector_world);

//        Eigen::Vector3d orientationD = vectorD / vectorD.norm();
//
//
//        Eigen::Vector3d k_x(1.5, 1.5, 1.0);
//        Eigen::Vector3d k_v(1.5, 1.5, 1.0);
//        Eigen::Vector3d k_q(1.5, 1.5, 1.0);
//        Eigen::Vector3d k_w(1.5, 1.5, 1.0);
//
//        Eigen::Vector3d A = -(cVector_world - cDesiredVector_world).cwiseProduct(k_x) -
//                            (vectorD - desiredVectorD).cwiseProduct(k_v) +
...
//        cDesiredOrientation_world = -A / A.norm();
//
//        Eigen::Vector3d desiredOrientationD(0, 0, 0);
//        if (!desiredVectorD.isZero())
//        {
//            desiredOrientationD = desiredVectorD / desiredVectorD.norm();
//        }
//        Eigen::Vector3d desiredOrientationDD(0, 0, 0);
//        if (!desiredVectorDD.isZero())
//        {
//            desiredOrientationDD = desiredVectorDD / desiredVectorDD.norm();
//        }
//
//        Eigen::Vector3d error = getOrientationError(cOrientation_world, cDesiredOrientation_world);
//        Eigen::Vector3d errorD = getOrientationErrorD(cOrientation_world, orientationD, cDesiredOrientation_world,
//                                                      desiredOrientationD);
//
//        Eigen::Vector3d F_n = A;
//        Eigen::Vector3d F_pd = getForcePD(error, errorD, k_q, k_w);
//
//        Eigen::Vector3d F_ff = getForceFF(massiveQuadcopter, length, cOrientation_world, orientationD,
//                                          cDesiredOrientation_world,
//                                          desiredOrientationD, desiredOrientationDD);
//        cout << desiredVectorD << endl;
//        float k_ff = 1;
//        Eigen::Vector3d F = F_n - F_pd - k_ff * F_ff;

//        Eigen::Vector3d moment = getMoment(F, rotationMatrix, omega);
//        Eigen::Vector4d revs = getRevs(F.norm(), moment);
//
//        cout << "recs:" << revs/ << endl;
//        TODO:pub revs!!!
//        cout << "F_ff:" << endl << error << endl;
//        cout << "F:" << endl << F << endl;

//        Eigen::Vector3d acceleration = F / massiveQuadcopter;


        Eigen::Vector3d k_p(-1,-1,-1);
        Eigen::Vector3d k_d(3,3,3);
//        Eigen::Vector3d output = -k_p * (cVector_world - cDesiredVector_world) + k_d * vectorD;
//        cout << output << endl;
        geometry_msgs::Twist targetError;
        targetError.linear.x = ((cVector_world - cDesiredVector_world).cwiseProduct(k_p) +  vectorD.cwiseProduct(k_d)).x();//output.x();
        targetError.linear.y = ((cVector_world - cDesiredVector_world).cwiseProduct(k_p) + vectorD.cwiseProduct(k_d)).y();//output.y();
